...
resource "aws_cloudwatch_log_group" "route_model" {
  name              = "/aws/lambda/${local.function_names.route_model}"
...
resource "aws_lambda_function" "route_model" {
  function_name = local.function_names.route_model
  ...
  depends_on = [aws_cloudwatch_log_group.route_model]
